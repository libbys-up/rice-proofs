...
(* Generalization of NEval_left_alias_persists_through_force from a nil
   starting guard to an ARBITRARY starting guard F0 -- same reason as
   NEval_left_alias_weaken_force_y_F's own generalization: needed so this
   works while forcing y from underneath a guard already carrying other
   in-progress locations, not just from a completely empty guard. *)
Lemma NEval_left_alias_persists_through_force_F :
  forall P x y F0, x <> y ->
  forall Gam Gmid v, NEval_left P F0 Gam (BExpr (EVar y)) Gmid v ->
  Gam x = Some (BExpr (EVar y)) ->
  Gmid x = Some (BExpr (EVar y)).
Proof.
  intros P x y F0 Hxy Gam Gmid v H Hgx.
  destruct (NEval_left_evar_shape P F0 Gam y Gmid v H) as
    [ [Hcase1 [HeqG _]]
    | [ [Hcase2 [HeqG _]]
      | [ [Hcase3 [HeqG _]]
  ...
  - subst Gmid.
    assert (HG1x : G1 x = Some (BExpr (EVar y)))
      by (exact (proj1 (NEval_left_alias_frozen P x y Hxy e0 Hne1 Hne2 Hne3 (y :: F0)
                   Gam e0 G1 v Hrec (or_introl eq_refl) Hgx Hz))).
    rewrite (hupd_neq G1 y v x Hxy). exact HG1x.
Qed.
...
Proof.
  ...
  - (* G_CaseChoice: xh's own content is a bare EChoice; the graph rewrites it
    ...
    destruct (IH (hupd Gam xh (BExpr (EVar yh))) HGam' HNVT' HNAL HWF' HCC' HAC'
                 x Hxdom' HGraphClosedYh Hxshape' Hreach2yh)
      as [Hxeq [HNVT1 [HWF1 [Gam1 [HHC1 [HCC1 [HAC1 [Hgx1 Hplug]]]]]]]].
  ...
  - (* G_CaseConFree: xh's own binding was a bare free variable (self-loop
    ...
    { intros w Hw.
      ...
      unfold G''. rewrite (hupd_list_notin ws (map (fun _ => GExpr EFree) ws) (hupd G0 xh (GExpr (ECon c1 ws))) w Hwws).
    ...
    { unfold G''. rewrite (hupd_list_notin ws (map (fun _ => GExpr EFree) ws) (hupd G0 xh (GExpr (ECon c1 ws))) x HxNotInWs).
    ...
    { unfold G''. rewrite (hupd_list_notin ws (map (fun _ => GExpr EFree) ws) (hupd G0 xh (GExpr (ECon c1 ws))) xh Hxnotinws).
      unfold hupd; rewrite Nat.eqb_refl; reflexivity. }
    ...
    { intros z g Hzg w Hw.
      ...
      - unfold G'' in Hzg. rewrite (hupd_list_notin ws (map (fun _ => GExpr EFree) ws) (hupd G0 xh (GExpr (ECon c1 ws))) z Hzws) in Hzg.
    ...
    destruct (IH (hupd_list (hupd Gam xh (BExpr (ECon c1 ws))) ws (map (fun w => BExpr (EVar w)) ws))
                 ...
                 x Hxdom' HGraphClosedExt Hxshape' Hreach2Body)
      as [Hxeq [HNVT1' [HWF1' [Gam1 [HHC1 [HCC1' [HAC1' [Hgx1 Hplug]]]]]]]].
    ...
    { rewrite (hupd_list_notin ws (map (fun w => BExpr (EVar w)) ws) (hupd Gam xh (BExpr (ECon c1 ws))) x HxNotInWs).
